...
export default class AnnotationDisplayStore {
  ...
  loadAnnotationDisplays = async (id_article: string) => {
    ...
    try {
      this.annotationDisplayArray =
        await agent.AnnotationDisplays.list(id_article)
  ...
  setSelectedAnnotationDisplayMap = async (id_instruct: number) => {
    let objects = this.annotationDisplayArray.filter(
      (item: AnnotationDisplay) => item.id_instruct === id_instruct,
    )
    if (objects) {
      this.selectedAnnotationDisplayMap.clear()
      runInAction(() => {
    ...
    try {
      ...
      runInAction(() => {
        objects.forEach((object) => {
          const i = this.annotationDisplayArray.findIndex(
            (x) =>
              x.id_article === object.id_article &&
              x.id_instruct === object.id_instruct &&
              x.id_annotation === object.id_annotation,
          )
          if (i !== -1) {
  ...
  deleteAnnotationDisplayArray = async (id_annotation: number) => {
    ...
    try {
      runInAction(() => {
        this.annotationDisplayArray = this.annotationDisplayArray.filter(
          (x) => x.id_annotation !== id_annotation,
        )
        this.selectedInstructionId !== 0 &&
          this.setSelectedAnnotationDisplayMap(this.selectedInstructionId)
        this.loading = false
      })
    } catch (error) {
      console.log(error)
